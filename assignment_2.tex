...
\begin{homeworkProblem}
    ...
    a) Use the Master method solve the following recurrence function:
    \begin{center}
    ...
    \begin{quoting}
        ...
        \end{align}
        \begin{center}
            $\therefore$ case 1 of master theorem \\
            The solution to the recurrence is $O(\log_2{n}^{\log_2{3}})$ after substituting for k.
        \end{center}
    \end{quoting}
    \vskip0.5cm
   %\begin{align}
   ...
   %\end{align}
    ...
    \vskip0.3cm
    \begin{quoting}
        Assuming that $T(1) = 1$ lets calculate the first few values of
        the recurrence to get an idea of the function.
        \begin{align}
            T(1) & = 1 \notag \\
            T(2) & = 2(1) + 1 = 3 \notag \\
            T(3) & = 2(3) + 1 = 7 \notag \\
        ...
        The initial values of the recurrence appears to be $2^{n} - 1$. Lets
        ...
        \end{center}
        \vskip0.5cm
        We have proven through induction that the recurrence has an upper 
...
\begin{homeworkProblem}
    ...
    \begin{algorithm}[H]
        \begin{algorithmic}[1]
            \Function{ReliablePath}{$G, w, s$}
            \State{} \Call{Initialize-Single-Source}{$G, s$} \Comment{Sets all but the root vertex to unreachable}
            \State{} $S = \emptyset$ \Comment{S is our final output, starts off as empty}
            \State{} $Q = G.V$ \Comment{Adds in the vertices we currently have a path to}
                \While{$Q \neq \emptyset$} \Comment{While we still have vertices that haven't been checked}
                \State{} $u = \Call{Extract-Max}{$Q$} $ \Comment{Removes the largest/most reliable vertex from $Q$}
                    \State{} $S = S \cup \{u\}$ \Comment{Mark the node we have just visited}
                    \For{each vertex $v \in G.Adj[u]$} \Comment{For each vertex with a path from our current vertex}
                        \State{} \Call{Tense}{$u,v,w$} \Comment{Inverse of relax, tries to increase value of the path}
                    \EndFor
    ...
    \begin{algorithm}[H]
        \begin{algorithmic}[1]
            ...
            \For{each vertex $v \in G.V$} \Comment{For every vertex within the graph}
                \State{} $v.d = \infty$ \Comment{We don't yet have a path to each vertex therefore $\infty$ for no route}
                \State{} $v.\pi = NIL$ \Comment{Again we haven't got a path so set all parents to nothing}
            \EndFor
            \State{} $s.d = 1$ \Comment{We don't have to travel anywhere from the root therefore $100\%$ reliability}
            \EndFunction
        \end{algorithmic}
    ...
    \begin{algorithm}[H]
        \begin{algorithmic}[1]
            \Function{Tense}{$u, v, w$} \Comment{This function is the inverse of relax, it aims to increase the path value}
                \If{$v.d < u.d * w(u,v)$} \Comment{Using product instead of sum, picks the larger value (Higher reliability)}
                    \State{} $v.d = u.d * w(u,v)$ \Comment{If our new path is better, save the value to the vertex}
                    \State{} $v.\pi = u$ \Comment{Update the parent to our current vertex}
                \EndIf
            \EndFunction{}
        \end{algorithmic}
    ...
    \begin{center}
        \textit{Extract-Max not shown given it vary's depending how it's implemented}
    \end{center}

    \textbf{Modified Dijkstra's Implementation}

    \begin{quoting} 
        
        Because this is a greedy algorithm it is always selecting outliers, the
        largest in this case, to step into next. There are two obvious ways we
        can go ahead and implement this algorithm, primitive arrays or the
        priority queue. \\

        The simplest to implement is the array method, which at every step
        would require the algorithm to check every vertex to find the max.
        This is an $O(|V|)$ operation, and given we will have to do it for
        every single node we visit it's clear to see it will become $0(|V|^2)$
        which we know is not a good complexity for large input sizes. \\

        Another option is to choose a priority queue which can be implemented
        using a max heap. All basic operations on a heap are $O(\log_2{|V|})$
        which after visiting every vertex and performing extract max would be
        $O(|V|\log_2{|V|})$. For large input sizes we can see that the priority
        is favorable. \\
    ...
    \begin{quoting}
        From the algorithm I have found the following paths to each node:
        \begin{align}
            B:&  \{A \to D \to B\} = 0.765 \notag \\
            C:&  \{A \to C\} = 0.9  \notag \\
            D:&  \{A \to D\} = 0.9  \notag \\
...
\begin{homeworkProblem}
    ...
    \begin{quoting}
        ...
        had a size of 9 and came from the following subsets:

        \begin{center}
            $s_1 = \{B,C,D,G\}, s_2 = \{A,E,F\}$
        \end{center}

        The order of these subsets is irrelevant as flipping the variables
        ...
        is defined as $2^{n} - 2$ however because when reversing
        the subsets the cuts that will be produced are the same we
        can half this to get: $2^{n}/2 -1$
        \begin{multicols}{2}
            \begin{tabular}{l l | }
                $\{B,C,D,E,F,G\}$ &$\{A\}$ \\
                $\{A,C,D,E,F,G\}$ &$\{B\}$ \\
    ...
    \begin{quoting}
        ...
        From there it will choose the vertices with the highest "internal"
        degree. If the cut is improved by swapping the vertices into the other
        subset it will accept that change. \\

        \textit{Internal degree} is the number of edges excluding any that have
        been removed by the cut. This is the \textbf{greedy criteria} for this
        algorithm with the largest being taken every time. \\

        Discussion on implementation of this algorithm follows the pseudo-code.
        ...
        \begin{algorithm}[H]
            \begin{algorithmic}[1]
                ...
                \State{} $subSet = \Call{Randomize-Sets}{V}$ \Comment{Split the vertices into a random pair of subsets.} \\

                \For{each $vertex$ in $V$} \Comment{For each vertex in the graph}
                    \For{each $vertex.Adj$ as $adj$} \Comment{For each connected node of that vertex.}
                        \If{\Call{In-Set}{$vertex, subSet$} $=$ \Call{In-Set}{$adj, subSet$}} \Comment{If they are in the same subset}
                            \State{} $innerDegree.vertex = innerDegree.vertex + 1$ \Comment{Increment innerDegree}
                        \EndIf{}
                    \EndFor{}
                \EndFor{} \\

                \For{each $vertex \in innerDegree$ in descending order} \Comment{\textbf{The greedy condition}}
                \State{}$oldSubSet = subSet$ \Comment{Keep track of last known cut}
                    \State{}$curCut = \Call{calculateCut}{V, subSet}$ \Comment{For comparing with the newly generated cut}

                    \If{$\Call{inSet}{vertex, subSet}$}
                        \State{}$subSet = subSet - \{vertex\}$ \Comment{Remove the current vertex from the subset}
                    \Else{}
                        \State{}$subSet = subSet \cup \{vertex\}$ \Comment{Add the current vertex to the subset}
                    \EndIf{} \\

                    \If{$\Call{calculateCut}{V, subSet} \le curCut$} \Comment{Revert changes if no improvement}
                        \State{}$subSet = oldSubSet$ \Comment{We haven't found a larger cut revert the changes}
                    \EndIf{}
                \EndFor{} \\

                \State{}\Return{$subSet$} \Comment{Only need $subSet$ since the other is $V - subSet$}
                \EndFunction{}
            \end{algorithmic}
        ...
        \begin{algorithm}[H]
            \begin{algorithmic}[1]
                \Function{In-Set}{$v, subSet$}
                    \State{}$retVal = false$ \Comment{To prevent double returns}
                    \If{$v \in subSet$} 
                        \State{}$retVAl = true$ \Comment{The vertex was found within the set}
                    \Else{}
                        \State{}$retVal = false$ \Comment{The vertex was not found within the set}
                    \EndIf{}
                    \State{}\Return{$retVal$}
                \EndFunction{}
            \end{algorithmic}
        ...
        \begin{algorithm}[H]
            \begin{algorithmic}[1]
                \Function{Calculate-Cut}{$V, subSet$}
                    \State{}$cut = 0$
                    \For{each vertex $v \in V$} \Comment{For each vertex in the graph}
                        \For{each edge $e \in v.Adj$} \Comment{For each vertex connected to our current vertex}
    ...
    \begin{quoting}
       ...
       Max Cut of a graph G. The pseudo-code that is shown is using set logic
       to describe operations that are being performed on these sets. Because
       of this the code looks far simpler than it would be finally implemented.
       There are two main ways that this information can be represented in the final
       implementation, each having a few trade offs which need to be weighed up
       relative to the situation. \\
       
       The first and simplest method is to use arrays, almost all programming
       languages have some form of arrays as a primitive data type. Arrays do
       have a downside with respect to the time complexity of operations on
       ...
       worst case scenario involves shuffling the entire array.  With small
       cases, much like the problem we have been given, this is not as big of
       an issue as the input size is low ($n = 7$). But for very large values
       of $n$ it can become very troublesome as we do several additions and
       deletions per input. \\

       Another option is linked lists, these aren't usually an existing data type
       in most programming languages but can be made by bolting together various
       other primitives. Linked lists unlike arrays are represented as individual
       ...
       arrangements of references to try and bring down the time complexity.
       The traditional linked list has references going in one directions with
       each only pointing to the next node. While this does give us $O(1)$
       complexity for insertion and deletion we will still require $O(n)$
       searching for each of these operations. We also aren't able to get back
       to the start of the list once we have started traversing it. A good
       variation of the linked list for this algorithm would have each node
       store the pointer to the next node along with a pointer to the head and
       tail node (The very first/last in the list). This would result in $O(1)$
       insertion and deletion, assuming we have a reference to the node
       already. \\
       ...
       then arrays are definitely a good choice. However in the majority of
       situations the computer running the program is going to have Gigabytes
       of memory and it's unlikely this algorithm will utilize all of that. \\
   \end{quoting}
   ...
   \begin{quoting}
       ...
       result given they only make choices at the local level.  \\

       This is part of the reason why I decided to use a random component
       within this algorithm, if the algorithm is presented with a graph
       ...
       problem solutions can easily be ranked by the \textit{CalculateCut}
   ...
   \begin{quoting}
       ...
       The output in the previous section shows a counter example of when
       the algorithm will not generate the optimal cut. Unlike some other
       algorithms because it is generating the initial sets randomly the
       result will change between runs, in this case with 63 different
       sub trees.

      %My algorithm particularly falls down when the subsets aren't even,
      %take the example of $s_1 = {G}, s_2 = {A, B, C, D, E, F}$, you can
      %see this is the worst possible case for the algorithm given the is
      %absolutely no internal links within $s_1$ and within $s_2$ they are
