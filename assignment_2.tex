...
\begin{homeworkProblem}
    ...
    \begin{quoting}
        ...
        \begin{center}
            ...
            The solution to the recurance is $O(\log_2{n}^{\log_2{3}})$.
        \end{center}
    \end{quoting}
    \vskip0.5cm
   %\begin{align}
...
\begin{homeworkProblem}
    ...
    \begin{algorithm}[H]
        \begin{algorithmic}[1]
            ...
            \State{} \Call{Initialize-Single-Source}{$G, s$} \Comment{Sets all but the root vertice to unreachable}
            ...
            \State{} $Q = G.V$ \Comment{Adds in the vertices we currently have a path to}
                ...
                \State{} $u = \Call{Extract-Max}{$Q$} $ \Comment{Removes the largest/most reliable vertex from $Q$}
                    ...
                    \For{each vertex $v \in G.Adj[u]$} \Comment{For each vertex with a path from our current vertex}
                        \State{} \Call{Tense}{$u,v,w$} \Comment{Inverse of relax, trys to increase value of the path}
                    \EndFor
    ...
    \begin{algorithm}[H]
        \begin{algorithmic}[1]
            ...
            \For{each vertex $v \in G.V$} \Comment{For every vertex within the graph}
                \State{} $v.d = \infty$ \Comment{We don't yet have a path to each vertice therefore $\infty$ for no route}
...
\begin{homeworkProblem}
   ...
   \begin{quoting}
       ...
       algorithms because it is generating the inital sets randomly the
